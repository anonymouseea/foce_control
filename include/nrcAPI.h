...
/**
 * @brief 数字IO端口输入(仅限虚拟IO板使用！)
 * @param port 要修改状态的数字输入IO端口，最大范围为 1 <= port <= 16，实际范围取决于所连接点IO模块上的IO端口数目
 * @param value 要输出的状态
 *    -   0  输出低电平
 *    -   1  输出高电平
 */
...
/**
 * @brief 模拟IO端口输入(仅限虚拟IO板使用！)
 * @param port 要修改的虚拟IO的模拟输入端口，最大范围为 1 <= port <= 16，实际范围取决于所连接点IO模块上的IO端口数目
 * @param value 要输出的数值
 */
...
/**
 * @brief 设置自定义IO aout配置
 * @note 设置自定义IO aout的数目和对应值
 * @param sum 自定义IO aout的数目，参数范围：sum >= 0
 * @param addr 自定义IO aout的对应值
 * @return
 */
...
/**
 * @brief 设置自定义IO ain配置
 * @note 设置自定义IO ain的数目和对应值
 * @param sum 自定义IO ain的数目，参数范围：sum >= 0
 * @param addr 自定义IO ain的对应值
 * @return
 */
...
/**
 * @brief 发送跟踪关节位置
 * @param target 关节目标位置 单位 度(长度为7)
 */
int NRC_Set_ServoJ_Pos(const std::array<double, 7>& target);
